{
    Original file (BufDataset.pas) was part of the Free Pascal run time library.
    Copyright (c) 1999-2006 by Joost van der Sluis, member of the
    Free Pascal development team

    ZMBufDataset is modified BudDataset implementation
...
type
  ...
  TResolverErrorEvent = procedure(Sender: TObject; DataSet: TZMBufDataset; E: EUpdateError;
...
{  OldValuesBuffer:
  ...
  public
    ...
    IndNr           : integer;
    constructor Create(const ADataset : TZMBufDataset); virtual;
  ...
  public
    ...
    FLastRecInd     : integer;
    FNeedScroll     : Boolean;
    constructor Create(const ADataset: TZMBufDataset); override;
    function ScrollBackward : TGetResult; override;
    function ScrollForward : TGetResult; override;
    function GetCurrent : TGetResult; override;
    function ScrollFirst : TGetResult; override;
...
type
  ...
  public
    ...
    procedure RestoreRecord(ADataset : TZMBufDataset); virtual; abstract;
    ...
    procedure StoreRecord(ADataset : TZMBufDataset; ARowState : TRowState; AUpdOrder : integer = 0); virtual; abstract;
  ...
  public
    ...
    procedure RestoreRecord(ADataset : TZMBufDataset); override;
    procedure StoreRecord(ADataset : TZMBufDataset; ARowState : TRowState; AUpdOrder : integer = 0); override;
    class function RecognizeStream(AStream : TStream) : boolean; override;
  end;

  TZMBufDataset = class(TDBDataSet)
  private
    FFileName: string;
...
constructor TZMBufDataset.Create(AOwner : TComponent);
begin
...
procedure TZMBufDataset.FetchAll;
begin
  repeat
  until (getnextpacket < FPacketRecords) or (FPacketRecords = -1);
end;

procedure TZMBufDataset.BuildIndex(var AIndex: TBufIndex);
...
function TZMBufDataset.GetIndexDefs : TIndexDefs;

begin
  Result := FIndexDefs;
end;

procedure TZMBufDataset.UpdateIndexDefs;
var i : integer;
begin
  FIndexDefs.Clear;
...
Function TZMBufDataset.GetCanModify: Boolean;
begin
  Result:= True;
end;

function TZMBufDataset.intAllocRecordBuffer: PChar;
begin
...
function TZMBufDataset.AllocRecordBuffer: PChar;
begin
...
procedure TZMBufDataset.FreeRecordBuffer(var Buffer: PChar);
begin
  ReAllocMem(Buffer,0);
end;

procedure TZMBufDataset.ClearCalcFields(Buffer: PChar);
begin
  if CalcFieldsSize > 0 then
    FillByte((Buffer+RecordSize)^,CalcFieldsSize,0);
end;

procedure TZMBufDataset.InternalOpen;
...
procedure TZMBufDataset.InternalClose;
...
procedure TZMBufDataset.InternalFirst;
begin
...
procedure TZMBufDataset.InternalLast;
begin
  FetchAll;
  with FCurrentIndex do
  SetToLastRecord;
...
constructor TBufIndex.Create(const ADataset: TZMBufDataset);
begin
  inherited create;
  FDataset := ADataset;
end;
...
procedure TZMBufDataset.CurrentRecordToBuffer(Buffer: PChar);
...
function TZMBufDataset.GetRecord(Buffer: PChar; GetMode: TGetMode; DoCheck: Boolean): TGetResult;
...
procedure TZMBufDataset.DoBeforeClose;
begin
...
function TZMBufDataset.GetActiveRecordUpdateBuffer : boolean;
...
procedure TZMBufDataset.ProcessFieldCompareStruct(AField: TField; var ACompareRec : TDBCompareRec);
begin
...
procedure TZMBufDataset.SetIndexFieldNames(const AValue: String);
begin
  if AValue<>'' then
    begin
...
procedure TZMBufDataset.SetIndexName(AValue: String);
var i : integer;
begin
...
procedure TZMBufDataset.SetMaxIndexesCount(const AValue: Integer);
begin
  CheckInactive;
  if AValue > 1 then
...
procedure TZMBufDataset.InternalSetToRecord(Buffer: PChar);
begin
  FCurrentIndex.GotoBookmark(PBufBookmark(Buffer+FRecordSize));
end;

procedure TZMBufDataset.SetBookmarkData(Buffer: PChar; Data: Pointer);
begin
  PBufBookmark(Buffer + FRecordSize)^.BookmarkData := pointer(Data^);
end;

procedure TZMBufDataset.SetBookmarkFlag(Buffer: PChar; Value: TBookmarkFlag);
begin
  PBufBookmark(Buffer + FRecordSize)^.BookmarkFlag := Value;
end;

procedure TZMBufDataset.GetBookmarkData(Buffer: PChar; Data: Pointer);
begin
  pointer(Data^) := PBufBookmark(Buffer + FRecordSize)^.BookmarkData;
end;

function TZMBufDataset.GetBookmarkFlag(Buffer: PChar): TBookmarkFlag;
begin
  Result := PBufBookmark(Buffer + FRecordSize)^.BookmarkFlag;
end;

procedure TZMBufDataset.InternalGotoBookmark(ABookmark: Pointer);
begin
...
function TZMBufDataset.getnextpacket : integer;
...
function TZMBufDataset.GetFieldSize(FieldDef : TFieldDef) : longint;
...
function TZMBufDataset.GetRecordUpdateBuffer(const ABookmark : TBufBookmark; IncludePrior : boolean = false; AFindNext : boolean = false): boolean;
...
function TZMBufDataset.GetRecordUpdateBufferCached(const ABookmark: TBufBookmark;
...
function TZMBufDataset.LoadBuffer(Buffer : PChar): TGetResult;
...
function TZMBufDataset.GetCurrentBuffer: PChar;
begin
...
function TZMBufDataset.GetFieldData(Field: TField; Buffer: Pointer;
  NativeFormat: Boolean): Boolean;
begin
  Result := GetFieldData(Field, Buffer);
end;

function TZMBufDataset.GetFieldData(Field: TField; Buffer: Pointer): Boolean;
...
procedure TZMBufDataset.SetFieldData(Field: TField; Buffer: Pointer;
...
procedure TZMBufDataset.InternalDelete;
var i         : Integer;
    StartInd  : Integer;
    RemRec    : pointer;
...
procedure TZMBufDataset.ApplyRecUpdate(UpdateKind : TUpdateKind);

begin
  raise EDatabaseError.Create(SApplyRecNotSupported);
end;

procedure TZMBufDataset.CancelUpdates;
...
procedure TZMBufDataset.SetOnUpdateError(const AValue: TResolverErrorEvent);

begin
  FOnUpdateError := AValue;
end;

procedure TZMBufDataset.ApplyUpdates; // For backwards-compatibility

begin
  ApplyUpdates(0);
end;

procedure TZMBufDataset.ApplyUpdates(MaxErrors: Integer);
...
procedure TZMBufDataset.InternalCancel;
...
procedure TZMBufDataset.InternalPost;
...
procedure TZMBufDataset.CalcRecordSize;
...
function TZMBufDataset.GetIndexFieldNames: String;
begin
...
function TZMBufDataset.GetIndexName: String;
begin
  result := FCurrentIndex.Name;
end;

function TZMBufDataset.GetRecordSize : Word;

begin
  result := FRecordSize + BookmarkSize;
end;

function TZMBufDataset.GetChangeCount: integer;
...
procedure TZMBufDataset.InternalInitRecord(Buffer: PChar);
...
procedure TZMBufDataset.SetRecNo(Value: Longint);
...
function TZMBufDataset.GetRecNo: Longint;
...
function TZMBufDataset.IsCursorOpen: Boolean;

begin
  Result := FOpen;
end;

Function TZMBufDataset.GetRecordCount: Longint;

begin
  Result := FBRecordCount;
end;

Function TZMBufDataset.UpdateStatus: TUpdateStatus;
...
function TZMBufDataset.GetNewBlobBuffer : PBlobBuffer;
...
function TZMBufDataset.GetNewWriteBlobBuffer : PBlobBuffer;
...
procedure TZMBufDataset.FreeBlobBuffer(var ABlobBuffer: PBlobBuffer);
...
begin
  FDataset := Field.DataSet as TZMBufDataset;
  if mode = bmread then
    begin
  ...
  else if mode=bmWrite then with FDataSet as TZMBufDataset do
    begin
...
function TZMBufDataset.CreateBlobStream(Field: TField; Mode: TBlobStreamMode): TStream;
...
procedure TZMBufDataset.AddIndex(const AName, AFields : string; AOptions : TIndexOptions; const ADescFields: string = '';
...
procedure TZMBufDataset.SaveToFile(AFileName: string;
  Format: TDataPacketFormat);
...
procedure TZMBufDataset.SetDatasetPacket(AReader: TDataPacketReader);
begin
...
procedure TZMBufDataset.GetDatasetPacket(AWriter: TDataPacketReader);
...
procedure TZMBufDataset.LoadFromStream(AStream: TStream; Format: TDataPacketFormat);
...
procedure TZMBufDataset.SaveToStream(AStream: TStream; Format: TDataPacketFormat);
var APacketReaderReg : TDatapacketReaderRegistration;
...
procedure TZMBufDataset.LoadFromFile(AFileName: string; Format: TDataPacketFormat);
var AFileStream : TFileStream;
begin
  if AFileName='' then AFileName := FFileName;
...
procedure TZMBufDataset.CreateDataset;
begin
  CheckInactive;
  CreateFields;
end;

function TZMBufDataset.BookmarkValid(ABookmark: TBookmark): Boolean;
begin
  Result:=FCurrentIndex.BookmarkValid(ABookmark);
end;

function TZMBufDataset.CompareBookmarks(Bookmark1, Bookmark2: TBookmark
  ): Longint;
begin
...
procedure TZMBufDataset.IntLoadFielddefsFromFile;
...
procedure TZMBufDataset.IntLoadRecordsFromFile;
...
procedure TZMBufDataset.InternalAddIndex(const AName, AFields : string; AOptions : TIndexOptions; const ADescFields: string;
                                       const ACaseInsFields: string);
...
procedure TZMBufDataset.DoFilterRecord({var}out Acceptable: Boolean);
begin
  Acceptable := true;
  // check user filter
...
procedure TZMBufDataset.SetFilterText(const Value: String);
begin
...
procedure TZMBufDataset.SetFiltered(Value: Boolean); {override;}
begin
...
function TZMBufDataset.Fetch: boolean;
begin
  // Empty procedure to make it possible to use TZMBufDataset as a memory dataset
  Result := False;
end;

function TZMBufDataset.LoadField(FieldDef: TFieldDef; buffer: pointer; out
...
function TZMBufDataset.IsReadFromPacket: Boolean;
begin
  Result := (FDatasetReader<>nil) or (FFileName<>'');
end;

procedure TZMBufDataset.ParseFilter(const AFilter: string);
begin
  // parser created?
...
Function TZMBufDataset.Locate(const KeyFields: string; const KeyValues: Variant; options: TLocateOptions) : boolean;
...
constructor TArrayBufIndex.Create(const ADataset: TZMBufDataset);
begin
...
const FpcBinaryIdent = 'BinZMBufDataset';
...
procedure TFpcBinaryDatapacketReader.RestoreRecord(ADataset: TZMBufDataset);
begin
  Stream.ReadBuffer(ADataset.GetCurrentBuffer^,ADataset.FRecordSize);
end;

procedure TFpcBinaryDatapacketReader.StoreRecord(ADataset: TZMBufDataset;
